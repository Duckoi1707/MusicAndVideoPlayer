...
""",
        ...
        if len(m.command) < 2:
            ...
            await m.delete()
            huehue = await m.reply("🔎 Tìm kiếm")
            query = m.text.split(None, 1)[1]
            search = ytsearch(query)
            if search == 0:
                await huehue.edit("`Không tìm thấy gì cho Truy vấn đã cho`")
            else:
                songname = search[0]
                url = search[1]
                duration = search[2]
                ...
                if hm == 0:
                    await huehue.edit(f"**YTDL LỖI ⚠️** \n\n`{ytlink}`")
                else:
                    if chat_id in QUEUE:
                        ...
                        await m.reply_photo(
                            photo=f"{thumbnail}",
                            caption=f"""
**#⃣ Thông tin bài hát yêu cầu {pos}
🏷️ Tên bài hát: [{songname}]({url})
        ...
        if replied.video or replied.document:
            ...
            if chat_id in QUEUE:
                ...
                await m.reply_photo(
                    photo="https://telegra.ph/file/d6f92c979ad96b2031cba.png",
                    caption=f"""
**#⃣ Video trong hàng đợi  {pos}
🏷️ Tên bài hát: [{songname}]({link})
💬 Trò chuyện ID: {chat_id}
🎧 Theo yêu cầu: {m.from_user.mention}**
""",
                )
            else:
...
""",
            ...
            await hmm.delete()
            await m.reply(
                f"➕ Thêm {lmt} Bài hát vào hàng đợi\n• Nhấp vào {HNDLR}danh sách phát Để xem danh sách phát**"
            )
        except Exception as e:
